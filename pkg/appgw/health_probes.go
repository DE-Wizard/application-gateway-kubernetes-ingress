...
func (c *appGwConfigBuilder) generateHealthProbe(backendID backendIdentifier) *n.ApplicationGatewayProbe {
	...
	if k8sProbeForServiceContainer != nil {
		if len(k8sProbeForServiceContainer.ProbeHandler.HTTPGet.Host) != 0 {
			probe.Host = to.StringPtr(k8sProbeForServiceContainer.ProbeHandler.HTTPGet.Host)
		}
		if len(k8sProbeForServiceContainer.ProbeHandler.HTTPGet.Path) != 0 {
			probe.Path = to.StringPtr(k8sProbeForServiceContainer.ProbeHandler.HTTPGet.Path)
		}
		if len(k8sProbeForServiceContainer.ProbeHandler.HTTPGet.Port.String()) != 0 {
			probe.Port = to.Int32Ptr(k8sProbeForServiceContainer.ProbeHandler.HTTPGet.Port.IntVal)
		}
		if k8sProbeForServiceContainer.ProbeHandler.HTTPGet.Scheme == v1.URISchemeHTTPS {
			probe.Protocol = n.ApplicationGatewayProtocolHTTPS
		}
		// httpGet schema is default to Http if not specified, double check with the port in case for Https
		if k8sProbeForServiceContainer.ProbeHandler.HTTPGet.Scheme == v1.URISchemeHTTP {
			if k8sProbeForServiceContainer.ProbeHandler.HTTPGet.Port.IntVal == 443 {
				probe.Protocol = n.ApplicationGatewayProtocolHTTPS
			} else {
...
func (c *appGwConfigBuilder) getProbeForServiceContainer(service *v1.Service, backendID backendIdentifier) *v1.Probe {
	...
	for _, container := range podList[0].Spec.Containers {
		for _, port := range container.Ports {
			...
			// found the container
			var probe *v1.Probe
			if container.ReadinessProbe != nil && container.ReadinessProbe.ProbeHandler.HTTPGet != nil {
				probe = container.ReadinessProbe
			} else if container.LivenessProbe != nil && container.LivenessProbe.ProbeHandler.HTTPGet != nil {
